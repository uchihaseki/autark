...
class PromptAgentEvaluator:
    async def evaluate(self, case: EvalCase, run_result: RunResult) -> EvalResult:
        ...
        if run_result.status == "error":
            scores = {"task_success": 0.0, "answer_quality": 0.0, "constraint_satisfaction": 0.0}
            scores["overall"] = weighted_overall(scores, {"task_success": 0.5, "answer_quality": 0.3, "constraint_satisfaction": 0.2})
            return EvalResult(
